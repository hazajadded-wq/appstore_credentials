...
import 'firebase_options.dart'; // Ensure you have this file generated by flutterfire

// ==========================================
// CONFIGURATION & CONSTANTS
// ==========================================
const String SERVER_API_URL =
    "https://lpgaspro.org/scgfs_notifications/notifications_api.php";
const String METHOD_CHANNEL_NAME = "com.pocket.salaryinfo/notifications";
...
class NotificationItem {
  ...
  factory NotificationItem.fromMySQL(Map<String, dynamic> map) {
    ...
    if (map['data_payload'] != null &&
      ...
      try {
        var decoded = jsonDecode(map['data_payload']);
...
class NotificationDb {
  static final NotificationDb instance = NotificationDb._internal();
  ...
  Future<Database> _open() async {
    ...
    final path = p.join(dbPath, 'scgfs_notifications_v2.db');
  ...
  Future<List<NotificationItem>> getAll() async {
    ...
    return maps.map((m) {
      return NotificationItem(
        ...
        data: m['dataJson'] != null ? jsonDecode(m['dataJson'] as String) : {},
...
// ==========================================
// NOTIFICATION MANAGER (Logic Hub)
// ==========================================
...
class NotificationManager extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    // 2. Listen to Native iOS calls (Foreground/Background fetch)
    _channel.setMethodCallHandler(_nativeMethodCallHandler);

    // 3. Sync with server API (Backup strategy)
    syncWithServer();
  }

  // Handle data coming from AppDelegate.swift
  Future<dynamic> _nativeMethodCallHandler(MethodCall call) async {
    if (call.method == "onNotificationReceived") {
      debugPrint("📱 [iOS Native] Received notification: ${call.arguments}");
      try {
        ...
        if (args is Map) {
          ...
          debugPrint("❌ Unknown arguments format");
          return;
        }
        ...
        debugPrint("❌ [iOS Native] Error parsing notification: $e");
      }
    }
  }
  ...
  Future<void> loadNotifications() async {
    try {
      _notifications = await NotificationDb.instance.getAll();
      _updateUnreadCount();
      notifyListeners();
      debugPrint('📂 [Manager] Loaded ${_notifications.length} from DB');
    } catch (e) {
      debugPrint('❌ [Manager] loadNotifications error: $e');
    }
  }

  // 🔄 Sync with MySQL Server (Backup for missed APNs)
  Future<void> syncWithServer() async {
    try {
      debugPrint('🔄 [Sync] Starting sync with MySQL server...');
      final response = await http
          .get(Uri.parse('$SERVER_API_URL?action=get_all&limit=50'))
          .timeout(const Duration(seconds: 10));

      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonResponse = jsonDecode(response.body);
        if (jsonResponse['success'] == true &&
            jsonResponse['notifications'] != null) {
          List<dynamic> serverList = jsonResponse['notifications'];
          bool hasChanges = false;

          for (var item in serverList) {
            try {
              NotificationItem serverItem = NotificationItem.fromMySQL(item);
              bool exists = _notifications.any((n) => n.id == serverItem.id);

              if (!exists) {
                debugPrint(
                    '📥 [Sync] New notification found: ${serverItem.title}');
                await NotificationDb.instance.save(serverItem);
                _notifications.insert(0, serverItem);
                hasChanges = true;
              }
            } catch (e) {
              debugPrint("❌ [Sync] Error parsing server item: $e");
            }
          }
          ...
          if (hasChanges) {
            ...
            _updateUnreadCount();
            notifyListeners();
            debugPrint('✅ [Sync] Notifications updated from server');
          } else {
            debugPrint('✅ [Sync] No new notifications');
          }
        }
      }
    } catch (e) {
      debugPrint('❌ [Sync] Failed to sync with server: $e');
    }
  }

  Future<void> addNotification(NotificationItem notification) async {
    if (!_notifications.any((n) => n.id == notification.id)) {
      await NotificationDb.instance.save(notification);
      _notifications.insert(0, notification);
      _notifications.sort((a, b) => b.timestamp.compareTo(a.timestamp));
      _updateUnreadCount();
      notifyListeners();
      debugPrint('📱 [Manager] Added notification: ${notification.id}');
    }
  }

  Future<void> addFirebaseMessage(RemoteMessage message) async {
    final n = NotificationItem.fromFirebaseMessage(message);
  ...
  Future<void> markAsRead(String notificationId) async {
    ...
    if (index != -1 && !_notifications[index].isRead) {
      await NotificationDb.instance.markRead(notificationId);
  ...
  Future<void> markAllAsRead() async {
    await NotificationDb.instance.markAllRead();
  ...
  Future<void> deleteNotification(String notificationId) async {
    await NotificationDb.instance.delete(notificationId);
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  // Note: iOS data is handled via AppDelegate method channel for background
}
...
void main() async {
  ...
  // Initialize DB first
  await NotificationDb.instance.database;
  ...
  try {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      debugPrint('📱 [FG] FCM: ${message.messageId}');
      await NotificationManager.instance.addFirebaseMessage(message);
    });

    // Tap listener
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      debugPrint('📱 [OPENED_FROM_NOTIF] ${message.messageId}');
      await NotificationManager.instance.addFirebaseMessage(message);
    ...
    // Initial message listener (Cold boot)
    final initialMessage = await messaging.getInitialMessage();
    if (initialMessage != null) {
      debugPrint('📱 [INITIAL_MESSAGE] ${initialMessage.messageId}');
                ...
                child: Column(
                  ...
                  children: [
                    _buildPrivacySection(
                      '1. المقدمة',
                      'تحترم الشركة العامة لتعبئة وخدمات الغاز خصوصية موظفيها وتل��زم بحماية بياناتهم الشخصية. توضح هذه السياسة كيفية جمع واستخدام وحماية المعلومات الخاصة بالموظفين.',
                    ),
                    _buildPrivacySection(
                      '2. البيانات المجمعة',
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  void initState() {
    super.initState();
    // Re-fetch notifications when opening this screen to ensure data is fresh
    NotificationManager.instance.syncWithServer();
  }
            ...
            child: ChangeNotifierBuilder<NotificationManager>(
              ...
              builder: (context, notificationManager, child) {
                ...
                return RefreshIndicator(
                  onRefresh: () async {
                    await NotificationManager.instance.syncWithServer();
                  },
                  color: const Color(0xFF00BFA5),
...
class _WebViewScreenState extends State<WebViewScreen> {
      ...
      controller!.setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (String url) {
            ...
            // Only apply auto-fit for HTML pages
            if (url.contains('.html')) {
              debugPrint(
                  '📄 HTML payslip page detected, preparing to show buttons');
              setState(() {
                zoomLevel = 1.0;
              });
          ...
          onNavigationRequest: (NavigationRequest request) {
            ...
            // Handle download URLs specially
            if (request.url.contains('download=1')) {
              debugPrint('📥 Download URL detected, removing parameter');
              String cleanUrl =
                  request.url.replaceAll(RegExp(r'[?&]download=1'), '');

              if (cleanUrl != currentUrl) {
                debugPrint('🔄 Loading clean URL: $cleanUrl');
            ...
            if (request.url == lastNavigatedUrl) {
              navigationCount++;
              debugPrint('⚠️ Duplicate navigation count: $navigationCount');

              if (navigationCount > 5) {
                debugPrint('🛑 Blocking excessive navigation');
                return NavigationDecision.prevent;
              }
            } else {
            ...
            debugPrint('✅ Allowing navigation (default)');
            return NavigationDecision.navigate;
          },
        ),
      );
      ...
      (function() {
        ...
        document.addEventListener('click', function(e) {
          ...
          if (target && target.tagName === 'A') {
            ...
            if (href && href.indexOf('download=1') !== -1) {
              e.preventDefault();
              var cleanUrl = href.replace(/[?&]download=1/g, '');
              window.location.href = cleanUrl;
              return false;
            }
          }
        ...
        document.addEventListener('submit', function(e) {
          ...
          if (action && action.indexOf('download=1') !== -1) {
            e.preventDefault();
            var cleanAction = action.replace(/[?&]download=1/g, '');
  ...
  bool _shouldShowButtons() {
    ...
    // إخفاء الأزرار من صفحة القائمة الرئيسية
    if (currentUrl == 'http://109.224.38.44:5000/payslip.html' ||
        currentUrl == 'http://109.224.38.44:5000/payslips' ||
        currentUrl == 'http://109.224.38.44:5000/salary' ||
        currentUrl.contains('/dashboard')) {
      debugPrint('🚫 This is main list page - hiding buttons');
      return false;
    }

    // إظهار الأزرار فقط عندما يكون هناك معرف أو parameter في الرابط
    // يعني أن المستخدم ضغط على "تحميل HTML" وفتح قسيمة محددة
    bool hasParameter = currentUrl.contains('?') ||
        currentUrl.contains('/view/') ||
        (currentUrl.contains('.html') &&
            currentUrl.split('/').last.length > 15);

    // أو إذا كان الرابط مختلف عن الرابط الأساسي
    bool isDifferentFromMain = currentUrl.contains('.html') &&
        currentUrl != 'http://109.224.38.44:5000/payslip.html';
